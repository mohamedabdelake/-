handler.before = async (m, { conn }) => {
    ...
    if (total >= 20) {
        ...
        await conn.sendMessage(m.chat, { 
            text: `🏆 الفائزون في المسابقة\n\n${sorted.join('\n')}\n\n🏅 @${winner.split('@')[0]} حصل على +500 نقطة خبرة و +10 كوكيز`,
            mentions
        });
    ...
    await m.reply(`✅ أحسنت! لديك الآن ${global.quiz.scores[m.chat][player]} نقطة`);
    handler(m, { conn });
};
...
async function handler(m, { conn }) {
    ...
    m.reply(`السؤال: ${q.question}\n\nاكتب الإجابة بسرعة ليتم احتساب النقطة. لديك 30 ثانية، وإلا ستنتهي اللعبة.`);
