const rewards = {
    daily: { xp: 100, cookies: 5, cooldown: 86400000, name: 'يومي', icon: '☀️', border: '☀️' },
    weekly: { xp: 500, cookies: 25, cooldown: 604800000, name: 'أسبوعي', icon: '📆', border: '📅' },
...
const handler = async (m, { conn, command }) => {
    ...
    if (lastClaim && (now - lastClaim) < reward.cooldown) {
        ...
        await m.reply(`❌ انتظر ${timeLeft} لاستلام الهدية ${reward.name}`);
        return;
    }
