...
let handler = async (m, { conn, bot, text }) => {
  try {
    if (!m.quoted?.mimetype) return m.reply("يرجى الرد على صورة أولاً.");
    if (!m.quoted.mimetype.startsWith('image/')) return m.reply("هذا الملف ليس صورة.");
